...
def get_target(source, target_dim, target_size, target_batch_size):
    ...
    if source_split[0] == 'mnist':
        ...
        #     if target.targets[i] == 9:
        #         indices.append(i)
        # idx = indices
        # target = Subset(target, [i for i in idx])
    ...
    # # plot a subset of the target
    # examples = [example[0].view(channel, target_dim, target_dim) for example in Subset(target, [i for i in range(64)])]
    # _, ax = plt.subplots(1)
    # ax.axis("off")
    # ax.imshow(np.transpose(utils.make_grid(examples, padding=0, normalize=True), (1, 2, 0)))
    data_loader = DataLoader(target, batch_size=target_batch_size, drop_last=True, num_workers=0,
...
# def record_test_sp_2(target_bacth, x, dim_target, save_dir, channel, epoc):
#     _, (ax1, ax2, ax3) = plt.subplots(1, 3)
#     t = torch.zeros([target_bacth.shape[0] * 2, target_bacth.shape[1], target_bacth.shape[2], target_bacth.shape[3]])
#     t[0::2] = target_bacth[:]
#     t[1::2] = x[:]
#     save_data(target_bacth, dim_target, ax1, channel, os.path.join(save_dir, str(epoc) + 'original_samples_ae.png'))
#     save_data(x, dim_target, ax2, channel, os.path.join(save_dir, str(epoc) + 'generated_samples_ae.png'))
#     save_data(t, dim_target, ax3, channel, os.path.join(save_dir, str(epoc) + 'all_together.png'))
#     return
